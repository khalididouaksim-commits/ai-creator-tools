...
export default function ImageGenerator() {
  ...
  useEffect(() => {
    addLog("تم تشغيل محرك الصور بنظام الدمج التلقائي في الوصف ✓");
  }, []);

  // دالة الرفع اللي كتحط الرابط تلقائياً داخل صندوق الوصف (Prompt) مع علامة +
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (data && data.status === "success" && data.data && data.data.url) {
        // تحويل الرابط إلى رابط مباشر وقابل للقراءة
        const directLink = data.data.url.replace("tmpfiles.org/", "tmpfiles.org/dl/");
        setRefUrl(directLink);
        
        // التعديل العبقري: كتابة الرابط تلقائياً داخل الـ textarea مع علامة +
        setPrompt((prevPrompt) => `${directLink} + ${prevPrompt}`);
        
        addLog("تم قفل الهوية وكتابة الرابط داخل صندوق الوصف ✓");
      } else {
        addLog("تنبيه: فشل استخراج الرابط المباشر");
      }
    } catch (err) {
      addLog("خطأ في الرفع، يرجى المحاولة مجدداً");
    }
    setUploading(false);
  };

  // دالة التوليد المباشرة اللي كتقرأ كاع داكشي اللي فصندوق الوصف (الرابط + النص)
  const generateImage = async () => {
    const cleanPrompt = prompt.trim();
    if (!cleanPrompt) { addLog("خطأ: صندوق الوصف فارغ!"); return; }
    if (uploading) { addLog("يرجى الانتظار حتى ينتهي رفع المرجع"); return; }
    
    setImageLoading(true);
    setImageUrl(null);
    addLog("جاري إرسال الوصف المدمج إلى محرك Flux...");
    
    let w = 1280, h = 720;
    if (aspectRatio === "9:16") { w = 720; h = 1280; }
    else if (aspectRatio === "1:1") { w = 1024; h = 1024; }
    
    // إرسال النص كامل (الرابط المكتوب + علامة + والوصف ديالك)
    const params = `?width=${w}&height=${h}&model=flux&enhance=true&nologo=true&seed=${customSeed}`;
    ...
    img.onload = () => {
      setImageUrl(fullUrl);
      addLog("تم إنتاج اللقطة الجديدة بنجاح ✓");
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(290px, 1fr))", gap: "16px" }}>
          ...
          <section style={{ background: "rgba(17,17,34,0.6)", borderRadius: "16px", padding: "16px", border: "1px solid rgba(139,92,246,0.15)" }}>
            ...
            <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="ارفع الصورة أولاً، وسيظهر الرابط هنا، ثم اكتب وراءه الملابس أو البيئة بالإنجليزية..." style={{ width: "100%", minHeight: "120px", padding: "12px", borderRadius: "10px", border: "1px solid #27274a", background: "#090915", color: "#f1f5f9", fontSize: "13px", fontFamily: "inherit" }} />
        ...
        <button onClick={generateImage} disabled={imageLoading || uploading} style={{ width: "100%", padding: "14px", background: "linear-gradient(135deg, #8b5cf6, #3b82f6)", color: "#fff", border: "none", borderRadius: "12px", fontWeight: "700", cursor: "pointer" }}>
          {imageLoading ? "🎨 جاري دمج المرجع نصياً وتوليد الصورة..." : "🚀 إنتاج اللقطة الجديدة المتطابقة تماماً"}
        </button>
      </div>
